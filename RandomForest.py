...
MODEL_NAME='RF_sample_mortgate'

params = {
    "n_estimators": 100, # 600 doesnt improve
    "max_depth": 25,
    "min_samples_split": 4,
    # "learning_rate": 0.05,
    # "loss": "squared_error",
...
# print(f'{MODEL_NAME}:{np.mean(all_mae)},{np.std(all_mae)}')
# print(f'{MODEL_NAME}:{np.mean(all_median)},{np.std(all_median)}')
# print(f'{MODEL_NAME}:{np.mean(all_mape)},{np.std(all_mape)}')
# import sys
# sys.exit()
...
fig.tight_layout()
plt.savefig(F'./uc/final_project_hp/figures/{MODEL_NAME}_boosting_feature_importance.png',dpi=300)
